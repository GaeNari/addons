...
function mod:Create(window)
	...
	window.bargroup.button:GetFontString():SetMaxLines(1)
...
function mod:Wipe(win)
	...
	win.bargroup:SetSortFunctionR(nil)
...
local function bar_order_sortR(a,b)
	return a and b and a.order and b.order and a.order > b.order
end
...
function mod:Update(win)
	...
	for i, data in ipairs(win.dataset) do
		if data.icon or (data.class and win.db.classicons) or (data.role and win.db.roleicons) then
			hasicon = true
		end
	end

	if win.totalvalue then
		--hasicon = true
		local bar = win.bargroup:GetBar("total summary")
		if not bar then
			bar = mod:CreateBar(win, "total summary", "", 10, 1, "Interface\\Icons\\Spell_Lightning_LightningBolt01")
			bar.id = "total summary"
			...
			bar:SetLabel(L["Total"])
			bar:SetTitle()
		end
		bar:SetTimerLabel(win.totalvalue)
		bar:SetValue(10)
	end
	...
	for i, data in ipairs(win.dataset) do
		if data.id then
			...
			if bar then
				if bar.class and ((data.class and bar.class ~= data.class and (win.db.classcolorbars or win.db.classicons)) or (data.role and bar.role ~= data.role and (win.db.rolecolorbars or win.db.roleicons))) then
					bar.class = data.class
					bar.role = data.role
					if data.color then
						-- Explicit color from dataset.
						bar:SetColor(data.color.r, data.color.g, data.color.b, data.color.a or 1)
					elseif win.db.classcolorbars then
					...
					if win.db.roleicons and data.role and data.role ~= "NONE" and mod.role_icon_tcoords[data.role] then
						bar:ShowIcon()
					...
					elseif win.db.classicons and mod.class_icon_tcoords[data.class] then
						bar:ShowIcon()
				...
				bar = mod:CreateBar(win, barid, "", 1, 1, data.icon, false)
				bar.id = barid
				bar.text = barlabel
				if data.icon then
					bar:ShowIcon()
				...
				if win.db.roleicons and data.role and data.role ~= "NONE" and mod.role_icon_tcoords[data.role] then
					...
					--bar:SetIconWithCoord("Interface\\LFGFrame\\UI-LFG-ICON-ROLES", GetTexCoordsForRole(data.role))
					bar:SetIconWithCoord("Interface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES", mod.role_icon_tcoords[data.role])
				elseif win.db.classicons and data.class and mod.class_icon_tcoords[data.class] then
					bar:ShowIcon()
					bar:SetIconWithCoord("Interface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes", mod.class_icon_tcoords[data.class])
				end
			...
			if win.metadata.ordersort then
				bar.order = i
			elseif win.metadata.ordercustomized then
				bar.order = data.order
			end

			if win.metadata.showspots and Skada.db.profile.showranks then
				if nr == 1 then
					previous_value = (data.realvalue or data.value)
					previous_rank = nr
				else
					if (data.realvalue or data.value) ~= previous_value then
						previous_value = (data.realvalue or data.value)
						previous_rank = nr
					end
				end
				bar:SetLabel(("%2u. %s"):format(previous_rank, data.label))
			else
	...
	if win.metadata.ordersort or win.metadata.ordercustomized then
		win.bargroup:SetSortFunction(bar_order_sort)
		win.bargroup:SetSortFunctionR(bar_order_sortR)
		win.bargroup:SortBars()
	else
		win.bargroup:SetSortFunction(nil)
		win.bargroup:SetSortFunctionR(nil)
