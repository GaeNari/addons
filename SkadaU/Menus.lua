...
function Skada:OpenMenu(window)
	...
	skadamenu.initialize = function(self, level)
		...
		if level == 1 then
			...
			info.func = function() InterfaceOptionsFrame_OpenToCategory("Skada: Ultimate") InterfaceOptionsFrame_OpenToCategory("Skada: Ultimate") end
			info.notCheckable = 1
			UIDropDownMenu_AddButton(info, level)
...
function Skada:CreateReportWindow(window)
	...
	if window then
		...
		Skada.db.profile.report.mode = window.selectedmode
	else
	...
	for i=1, #list, 2 do
		local chan = list[i+1]
		if chan then
			channellist[#channellist + 1] = chan
			channellist[#channellist + 1] = chan..L[" (Channel)"]
			channelvalue[chan] = "channel"
		end
	end
	...
	for i = 1, #channellist, 2 do
		channelbox:AddItem(channellist[i], channellist[i+1])
	end
	channelbox:SetCallback("OnValueChanged",
			function(f, e, value)
				Skada.db.profile.report.channel = value
				Skada.db.profile.report.chantype = channelvalue[value]
			end
		)
	...
	whisperbox:SetLabel(L["Name of recipient"])
	...
	report:SetCallback("OnClick", function()
		...
		if channel and channel ~= "" and chantype and mode and set and number then
			Skada:Report(channel, chantype, mode, set, number, window)
		else
			Skada:Print(L["Error: No target selected"])
		end
