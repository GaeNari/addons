...
do
	...
	function lib:NewBarGroup(name, orientation, height, length, thickness, frameName)
		...
		list.button:GetFontString():SetMaxLines(1)
...
function barListPrototype:SetSortFunctionR(func)
	if func then
		assert(type(func) == "function")
	end
	self.sortFuncR = func
...
function barListPrototype:GetSortFunctionR(func)
	return self.sortFuncR
end
...
do
	...
	function barListPrototype:SortBars()
		...
		if growup then
			table_sort(values, self.sortFuncR or sortFuncR)
			from = "BOTTOM"
			to = "TOP"
		...
		for i = start, stop, step do
			...
			if shown <= maxbars then
				...
				if orientation == 3 then
					v:SetPoint(from.."RIGHT", lastBar, to.."RIGHT", -x1, y1)
					v:SetPoint(from.."LEFT", lastBar, to.."LEFT", -x2, y2)
				else
...
function barPrototype:SetTitle()
	self.value = 10
	self.maxValue = 1
	self.isTitle = true
end

function barPrototype:SetUnTitle()
	self.displayMax = nil
	self.isTitle = nil
end
...
function barPrototype:UpdateColor()
	...
	if map then
		self.texture:SetVertexColor(map[0], map[1], map[2], map[3])
	end
end
