...
Skada.windowdefaults = {
	...
	title = {height = 15, font="Arial Narrow", fontsize=11,margin=0, texture="Aluminium", bordertexture="None", borderthickness=2, color = {r=0.1,g=0.1,b=0.3,a=0.8}, fontflags = ""},
			...
			windows = {
				...
				args = {
					...
					create = {
						...
						set=function(self, val) if val and val ~= "" then Skada:CreateWindow(val, nil, newdisplay) Skada:ApplySettings() end end,
						order=1,
					},
			...
			generaloptions = {
				...
				args = {
					...
					showtotals = {
							...
							set=function() Skada.db.profile.showtotals = not Skada.db.profile.showtotals Skada:ApplySettings() end,
					},
					...
					numberformat = {
						...
						values=	function()
									if GetLocale() == "koKR" then
										return {[1] = L["Condensed"], [2] = L["Condensed(Not DPS)"], [3] = L["Condensed(Not DPS+No Comma)"], [4] = L["M/K"], [5] = L["M/K(Not DPS)"], [6] = L["M/K(Not DPS+No Comma)"], [7] = L["Detailed"], [8] = L["Detailed(No comma)"]} 
									else
										return {[1] = L["Condensed"], [2] = L["Condensed(Not DPS)"], [3] = L["Condensed(Not DPS+No Comma)"], [7] = L["Detailed"], [8] = L["Detailed(No comma)"]} 
									end
								end,
						get=function() return Skada.db.profile.numberformat end,
						set=function(self, opt) Skada.db.profile.numberformat = opt Skada:UpdateDisplay(true) end,
						order=11,
					},
					...
					tenativecombatstart = {
							...
							order=10,
							get=function() return Skada.db.profile.tentativestart end,
							set=function() Skada.db.profile.tentativestart = not Skada.db.profile.tentativestart end,
					},

					showrealm = {
							type="toggle",
							name=L["Show realm name"],
							desc=L["Show realm name in modes"],
							order=10.5,
							get=function() return Skada.db.profile.showrealm end,
							set=function()
										Skada.db.profile.showrealm = not Skada.db.profile.showrealm 
										Skada:ApplySettings()
									end,
					},
