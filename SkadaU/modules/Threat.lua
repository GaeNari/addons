Skada:AddLoadableModule("Threat", function(Skada, L)
	...
	local mod = Skada:NewModule(L["Threat list"])

	mod.Order = 101
	mod.NoTitleModify = true

	local modname = mod:GetName()
	local UnitAffectingCombat, InCombatLockdown, UnitExists, UnitIsFriend, UnitClass, IsInRaid, GetNumGroupMembers = UnitAffectingCombat, InCombatLockdown, UnitExists, UnitIsFriend, UnitClass, IsInRaid, GetNumGroupMembers
	...
	local function add_to_threattable(win, uId, target)
		if uId and UnitName(uId) then
			local isTanking, status, threatpct, rawthreatpct, threatvalue = UnitDetailedThreatSituation(uId, target)

			if Skada.db.profile.modules.threatraw then
				if threatvalue then

					local name = Skada.db.profile.showrealm and GetUnitName(uId, true) or UnitName(uId)
	...
	local function getTPS(threatvalue)
		if Skada.current then
			...
			return threatvalue / math.max(1,totaltime)
		else
	...
	function mod:SpecialUpdate(win, set)
		local target = nil
		...
		if target then
			...
			for i = 1, GetNumGroupMembers() do
				add_to_threattable(win, uId..i, target)

				if UnitExists(uId..i.."pet") then
					add_to_threattable(win, uId..i.."pet", target)
				end
			end
			...
			if not IsInRaid() then
				...
				if UnitExists("pet") then
					add_to_threattable(win, "pet", target)
				end
			end

			local tankName = Skada.db.profile.showrealm and GetUnitName(target.."target", true) or UnitName(target.."target")
			if tankName and not threatList[tankName] then
				add_to_threattable(win, target.."target", target)
			end

			if #win.dataset ~= nr then
				...
				if data.id then
					...
					if data.threat and data.threat > 0 then
						...
						data.valuetext = Skada:FormatValueText(
														Skada:FormatNumber(data.threat), self.metadata.columns.Threat,
														Skada:FormatNumber(getTPS(data.threat)), self.metadata.columns.TPS,
	...
	function mod:OnEnable()
		mod.metadata = {wipestale = 1, columns = {Threat = true, TPS = false, Percent = true}}
