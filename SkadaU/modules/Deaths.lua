Skada:AddLoadableModule("Deaths", function(Skada, L)
	...
	local function log_deathlog(set, playerid, playername, srcname, spellid, spellname, amount, absorb, overkill, timestamp, logoverride, healthoverride)
		local player = Skada:get_player(set, playerid, playername, true)
		if player then
	...
	local function log_death(set, playerid, playername, timestamp)
		...
		if player then
			...
			log_deathlog(set, playerid, playername, nil, spellid, spellname, nil, nil, nil, timestamp, nil, 0)
	...
	local function log_resurrect(set, playerid, playername, srcname, spellid, spellname, timestamp)
		...
		if player and player.deaths and player.deaths[1] then
			log_deathlog(set, playerid, playername, srcname, spellid, spellname, nil, nil, nil, timestamp, player.deaths[1].log, 0)
		end
	end
	...
	local function log_SORdeath(set, playerid, playername, timestamp)
		...
		-- Add log entry to to previous death.
		if player and player.deaths and player.deaths[1] then
			log_deathlog(set, playerid, playername, playername, spellid, spellname, nil, nil, nil, timestamp, player.deaths[1].log, 0)
		end
	...
	local function Missed(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
		...
		if dstGUID and timestamp == SORtime[dstGUID] then -- this is actually the killing blow for the SOR we just recorded
			...
			log_deathlog(Skada.total, dstGUID, dstName, srcName, spellId, nil, samount and -samount, nil, nil, timestamp, nil, 0)
		end
	end

	local function Resurrect(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool)
		log_resurrect(Skada.total, dstGUID, dstName, srcName, spellId, nil, timestamp)
	end

	local function Absorbed(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
		local chk = ...
		...
		log_deathlog(Skada.total, dstGUID, dstName, srcName, spellId or 88163, nil, nil, aAmount and -aAmount, nil, timestamp)
	end
	...
	local function SpellDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, samount, soverkill, sschool, sresisted, sblocked, sabsorbed, scritical, sglancing, scrushing)
		log_deathlog(Skada.total, dstGUID, dstName, srcName, spellId, nil, samount and -samount, sabsorbed and -sabsorbed, soverkill, timestamp)
	end
	...
	local function EnvironmentalDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, environmentalType, samount, soverkill, sschool, sresisted, sblocked, sabsorbed, scritical, sglancing, scrushing)
		log_deathlog(Skada.total, dstGUID, dstName, srcName, nil, environmentalType, samount and -samount, sabsorbed and -sabsorbed, soverkill, timestamp)
	end

	local function Instakill(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool)
		spellId = spellId or 80468

		log_deathlog(Skada.total, dstGUID, dstName, srcName, spellId, spellName, -9999999, nil, 9999999, timestamp)
	end

	local function SpellHeal(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, samount, soverhealing, absorbed, scritical)
		samount = max(0, samount - soverhealing)

		log_deathlog(Skada.total, dstGUID, dstName, srcName, spellId, nil, samount, absorbed, 0, timestamp)
	end
			...
			if player.deaths and #player.deaths > 0 then
				...
				if set == Skada.total then
					...
					d.valuetext = Skada:FormatValueText(
						Skada:FormatHitNumber(#player.deaths), self.metadata.columns.Deaths
						)
				...
				if spellid then
					d.label = (Skada.db.profile.showrealm and player.name or player.shortname) .. ": " .. (spellname or GetSpellInfo(spellid))
				else 
	...
	function deathlog:Enter(win, id, label)
		...
		self.title = L["Death"]..": "..player.name
	end
	...
	local function log_tooltip(win, id, label, tooltip)
		if label and tooltipTable[id] then
			tooltip:AddDoubleLine(label, tooltipTable[id], 255,255,255,255,255,255)
		end
	end
			...
			for i, death in ipairs(player.deaths) do
				...
				for j, log in ipairs(death.log) do
					...
					if diff > -60 then
						...
						if log.srcname then 
							local srcname = Skada.db.profile.showrealm and log.srcname or log.srcname:gsub("%-.*$", "")
							label = label..srcname..L["'s "]
						end
						...
						local overkill = log.overkill or 0
						if self.metadata.columns.Change then
							local change = Skada:FormatHitNumber(math.abs(amount), true)
							if amount > 0 then
								change = "+"..change
						...
						if (absorb ~= 0 or overkill ~= 0) and self.metadata.columns.Change then
							if overkill > 0 then
								amt = amt.." ("..Skada:FormatHitNumber(math.abs(overkill), true).." "..L["Overkill"]..")"
							elseif absorb ~= 0 then
								amt = amt.." ("..Skada:FormatHitNumber(math.abs(absorb), true).." "..ABSORB..")"
							end
						end
	...
	function mod:OnEnable()
		...
		deathlog.metadata 	= {ordersort = true, tooltip = log_tooltip, columns = {Change = true, Health = false, Percent = true}}
		...
		Skada:RegisterForCL(Absorbed, 'SPELL_ABSORBED', {dst_is_not_interesting_nopets = true})
	...
	function mod:GetSetSummary(set)
		return Skada:FormatHitNumber(set.deaths)
	end
