--
-- JSHB Warlock - dispel module
--

if (select(2, UnitClass("player")) ~= "WARLOCK") then return end

local dispelTexture = "Interface\\Icons\\Spell_Nature_Purge" -- Devour Magic
		...
		if (JSHB.db.profile.dispel.removednotify == true) then
			...
			JSHB.F.DispelAlert:SetScript("OnEvent",
				function(self, event, ...)
					...
					if (self._subEvent == "SPELL_DISPEL")
						and ((self._sourceGUID == UnitGUID("player")) or (bit.band(self._sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == 1))
						and (self._destGUID ~= UnitGUID("pet")) then
