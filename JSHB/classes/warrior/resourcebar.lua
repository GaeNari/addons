--
-- JSHB Warrior - resource bar module
--
...
function JSHB.SetupResourceBarModule()
	...
	if JSHB.db.profile.resourcebar.enabled then
		...
		-- Create the Frame
		JSHB.F.ResourceBar = JSHB.MakeFrame(JSHB.F.ResourceBar, "StatusBar", "JSCM_RESOURCEBAR", JSHB.db.profile.resourcebar.anchor[2] or UIParent)
		...
		--JSHB.F.ResourceBar:RegisterEvent("UNIT_ATTACK_SPEED", "player")
		...
		JSHB.F.ResourceBar:SetScript("OnEvent",
			function(self, event, ...)
				...
				--	JSHB.SetupResourceBarModule() -- need to change this to update the shotbar only
				end
			end)
	...
	JSHB.F.StackBarsHost = JSHB.MakeFrame(JSHB.F.StackBarsHost, "Frame", "JSCM_STACKBARS_HOST", JSHB.db.profile.resourcebar.embedstacks and JSHB.F.ResourceBar or (JSHB.db.profile.resourcebar.anchor_stacks[2] or UIParent) )
	...
	if (GetSpecialization() == 1) then -- Arms
		--	
		-- Arms Warrior do not have baseline stacking or proc based buffs
		--
	elseif (GetSpecialization() == 2) then -- Fury
		numBars = 2
		checkStacksFunction = function(self) return(select(4, UnitAura("player", GetSpellInfo(46916), nil, "HELPFUL") ) or 0) end -- Blood Surge
		checkFunction = Check_AlwaysFlash
	elseif (GetSpecialization() == 3) then -- Protection
		--	
		-- Protection Warrior do not have baseline stacking or proc based buffs
		--
	end
