--
-- JSHB Warrior - dispel module
--

if (select(2, UnitClass("player")) ~= "WARRIOR") then return end

local dispelTexture = "Interface\\Icons\\INV_Shield_05" -- Shield Slam
local DISPEL_UPDATEINTERVAL = 0.1

function JSHB.SetupDispelModule(lockName)
	if (JSHB.IsGlyphActive(58375) == false) then return end -- make sure the player has Glyph of Shield Slam active.
	...
	if ( (not lockName) or (lockName == "MOVER_DISPELALERT") ) then
		-- Create the Frame
		JSHB.F.DispelAlert = JSHB.MakeFrame(JSHB.F.DispelAlert, "Frame", "JSCM_DISPELALERT", JSHB.db.profile.dispel.anchor[2] or UIParent)
	...
	if ((not lockName) or (lockName == "MOVER_DISPELREMOVABLES")) then
		...
		JSHB.F.DispelAlertRemovables = JSHB.MakeFrame(JSHB.F.DispelAlertRemovables, "Frame", "JSCM_DISPELALERT_REMOVABLES", JSHB.db.profile.dispel.anchor_removables[2] or UIParent)
