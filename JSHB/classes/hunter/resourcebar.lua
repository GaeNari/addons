...
local function updatePrediction()
	...
	if JSHB.db.profile.resourcebar.enableprediction and JSHB.F.ResourceBar.playerIsCasting then
		...
		JSHB.F.ResourceBar.PredictionFrame:SetSize(
			(JSHB.F.ResourceBar:GetWidth() / 100) * newPrediction1, JSHB.F.ResourceBar:GetHeight() )

		JSHB.F.ResourceBar.PredictionFrame:ClearAllPoints()
		JSHB.F.ResourceBar.PredictionFrame:SetPoint("LEFT", JSHB.F.ResourceBar, "LEFT",
			JSHB.F.ResourceBar:GetWidth() / (select(2, JSHB.F.ResourceBar:GetMinMaxValues() ) / JSHB.F.ResourceBar:GetValue() ), 0)

		if (UnitPower("player") + newPrediction1) > UnitPowerMax("player") then
			JSHB.F.ResourceBar.PredictionFrame:SetSize(
				(JSHB.F.ResourceBar:GetWidth() / 100) * (UnitPowerMax("player") - UnitPower("player") ), JSHB.F.ResourceBar:GetHeight() )			
		end
