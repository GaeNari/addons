--
-- JSHB Mage - crowd control module
--
	...
	JSHB.F.CrowdControl.ccFrame[1]:SetScript("OnEvent",
		function(s, event, ...)
			...
			if (s._subEvent == "SPELL_AURA_APPLIED") then
				-- Frost Nova
				if (s._spellId == 122) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.frostnova) then
					addAOETimer(122, 8)
				-- Polymorph
				elseif (s._spellId == 118) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.polymorph) then
					addCCTimer(118, s._destGUID, 50)
				-- Ring of Frost
				elseif (s._spellId == 82691) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.ringoffrost) then
					addAOETimer(82691, 10)
				-- Slow
				elseif (s._spellId == 31589) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.slow) then
					addCCTimer(31589, s._destGUID, 15)
				end
			elseif (s._subEvent == "SPELL_AURA_REFRESH") then
				-- Frost Nova
				if (s._spellId == 122) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.frostnova) then
					refreshAOETimer(122, 8)
				-- Polymorph
				elseif (s._spellId == 118) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.polymorph) then
					refreshCCTimer(118, s._destGUID, 50)
				-- Ring of Frost
				elseif (s._spellId == 82691) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.ringoffrost) then
					refreshAOETimer(82691, 10)
				-- Slow
				elseif (s._spellId == 31589) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.slow) then
					refreshCCTimer(31589, s._destGUID, 15)
				end
			elseif (s._subEvent == "SPELL_AURA_REMOVED") then
				-- Frost Nova
				if (s._spellId == 122) and (s._sourceGUID == UnitGUID("player")) then
					stopAOETimer(122)
				-- Polymorph
				elseif (s._spellId == 118) and (s._sourceGUID == UnitGUID("player")) then
					stopCCTimer(118, s._destGUID)
				-- Ring of Frost
				elseif (s._spellId == 82691) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.ringoffrost) then
					stopAOETimer(82691)
				-- Slow
				elseif (s._spellId == 31589) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.slow) then
					stopCCTimer(31589, s._destGUID, 15)
				end
			end	
		end)
