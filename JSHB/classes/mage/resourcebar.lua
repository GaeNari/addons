...
function JSHB.SetupResourceBarModule()
	...
	if (GetSpecialization() == 1) then -- Arcane
		numBars = 4
		checkStacksFunction = function(self) return(select(4, UnitAura("player", GetSpellInfo(36032), nil, "HARMFUL") ) or 0) end -- Arcane Charge
		checkFunction = Check_FlashAtMax
	elseif (GetSpecialization() == 2) then -- Fire
		--	
		-- Fire Mage
		--
	elseif (GetSpecialization() == 3) then -- Frost
		numBars = 2
		checkStacksFunction = function(self) return(select(4, UnitAura("player", GetSpellInfo(44544), nil, "HELPFUL") ) or 0) end -- Fingers of Frost
		checkFunction = Check_FlashAtMax
	end
