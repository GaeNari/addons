--
-- JSHB Mage - dispel module
--

if (select(2, UnitClass("player")) ~= "MAGE") then return end

local dispelTexture = "Interface\\Icons\\Spell_Arcane_Arcane02" -- Spellsteal
			...
			function(self, elapsed)
				...
				if (not UnitExists("target") ) or (not (UnitReaction("player", "target") ) ) or (UnitExists("target") and (UnitReaction("player", "target") > 4) ) or UnitIsDeadOrGhost("player") then
					...
					for i=1,40 do			
						...
						if (self._debuffType == "Magic") and self._isStealable then
		...
		if (JSHB.db.profile.dispel.removednotify == true) then
			...
			JSHB.F.DispelAlert:SetScript("OnEvent",
				function(self, event, ...)
					...
					if (self._subEvent == "SPELL_STOLEN") and (self._sourceGUID == UnitGUID("player") ) and (self._destGUID ~= UnitGUID("pet") ) then
