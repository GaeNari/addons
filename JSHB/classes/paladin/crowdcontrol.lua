--
-- JSHB Paladin - crowd control module
--
	...
	JSHB.F.CrowdControl.ccFrame[1]:SetScript("OnEvent",
		function(s, event, ...)
			...
			if (s._subEvent == "SPELL_AURA_APPLIED") and (s._sourceGUID == UnitGUID("player")) then
				-- Blinding Light
				if (s._spellId == 105421) and (JSHB.db.profile.crowdcontrol.blindinglight) then
					addAOETimer(105421, 6)
				-- Fist of Justice
				elseif (s._spellId == 105593) and (JSHB.db.profile.crowdcontrol.fistofjustice) then
					addCCTimer(105593, s._destGUID, 6)
				-- Hammer of Justice
				elseif (s._spellId == 853) and (JSHB.db.profile.crowdcontrol.hammerofjustice) then
					addCCTimer(853, s._destGUID,6)
				-- Repentance
				elseif (s._spellId == 20066) and (JSHB.db.profile.crowdcontrol.repentance) then
					addCCTimer(20066, s._destGUID, 60)
				-- Turn Evil
				elseif (s._spellId == 10326) and (JSHB.db.profile.crowdcontrol.turnevil) then
					addCCTimer(10326, s._destGUID, 40)
				end
			elseif (s._subEvent == "SPELL_AURA_REFRESH") and (s._sourceGUID == UnitGUID("player")) then
				-- Fist of Justice
				if (s._spellId == 105593) and (JSHB.db.profile.crowdcontrol.fistofjustice) then
					refreshCCTimer(105593, s._destGUID,6)
				-- Hammer of Justice
				elseif (s._spellId == 853) and (JSHB.db.profile.crowdcontrol.hammerofjustice) then
					refreshCCTimer(853, s._destGUID,6)
				-- Repentance
				elseif (s._spellId == 20066) and (JSHB.db.profile.crowdcontrol.repentance) then
					refreshCCTimer(20066, s._destGUID, 60)
				-- Turn Evil
				elseif (s._spellId == 10326) and (JSHB.db.profile.crowdcontrol.turnevil) then
					refreshCCTimer(10326, s._destGUID,40)
				end
			elseif (s._subEvent == "SPELL_AURA_REMOVED") and (s._sourceGUID == UnitGUID("player")) then
				-- Blinding Light
				if (s._spellId == 105421) then
					stopAOETimer(105421)
				-- Fist of Justice
				elseif (s._spellId == 105593) then
					stopCCTimer(105593, s._destGUID)
				-- Hammer of Justice
				elseif (s._spellId == 853) then
					stopCCTimer(853, s._destGUID)
				-- Repentance
				elseif (s._spellId == 20066) then
					stopCCTimer(20066, s._destGUID)
				-- Turn Evil
				elseif (s._spellId == 10326) then
					stopCCTimer(10326, s._destGUID)
				end
			end	
		end)
