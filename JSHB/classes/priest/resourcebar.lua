--
-- JSHB Priest - resource bar module
--
...
function JSHB.SetupResourceBarModule()
	...
	if JSHB.db.profile.resourcebar.enabled then
		...
		if JSHB.db.profile.resourcebar.resourcenumber then
			...
			JSHB.F.ResourceBar.value:SetPoint("CENTER", JSHB.F.ResourceBar, "CENTER", JSHB.db.profile.resourcebar.resourcefontoffset, 0)
		...
		if JSHB.db.profile.resourcebar.targethealth then		
			...
			JSHB.F.ResourceBar.targetHealthValue:SetPoint("LEFT", JSHB.F.ResourceBar, "LEFT", 1 + JSHB.db.profile.resourcebar.healthfontoffset, 0)
	...
	if (GetSpecialization() == 1) then -- Discipline
		numBars = 5 -- Evangelism stacks
		checkStacksFunction = function(self) return(select(4, UnitAura("player", GetSpellInfo(81662), nil, "HELPFUL") ) or 0) end -- Evangelism
		checkFunction = Check_FlashAtMax
	elseif (GetSpecialization() == 2) then -- Holy
		numBars = 2 -- Serendipity
		checkStacksFunction = function(self) return(select(4, UnitAura("player", GetSpellInfo(63733), nil, "HELPFUL") ) or 0) end -- Serendipity
		checkFunction = Check_FlashAtMax
	elseif (GetSpecialization() == 3) then -- Shadow
		numBars = UnitPowerMax("player", SPELL_POWER_SHADOW_ORBS) -- Shadow Orbs
		checkStacksFunction = function(self) return(UnitPower("player", SPELL_POWER_SHADOW_ORBS) or 0) end -- Shadow Orbs
		checkFunction = Check_FlashAtMax
	end
