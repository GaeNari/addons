--
-- JSHB Priest - crowd control module
--
	...
	JSHB.F.CrowdControl.ccFrame[1]:SetScript("OnEvent",
		function(s, event, ...)
			...
			if (s._subEvent == "SPELL_AURA_APPLIED") then
				-- Psychic Scream
				if (s._spellId == 8122) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.psychicscream) then
					addAOETimer(8122, 8)
				-- Shackle Undead
				elseif (s._spellId == 9484) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.shackleundead) then
					addCCTimer(9484, s._destGUID, 50)
				-- Void Tendrils
				elseif (s._spellId == 114404) and (bit.band(s._sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == 1) and (JSHB.db.profile.crowdcontrol.voidtendrils) then
					addAOETimer(114404, 20)
				end
			elseif (s._subEvent == "SPELL_CAST_SUCCESS") then
				-- Dominate Mind
				if (s._spellId == 605) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.dominatemind) then 
					addCCTimer(605, s._destGUID, 30)
				end
			elseif (s._subEvent == "SPELL_AURA_REFRESH") then
				-- Shackle Undead
				if (s._spellId == 9484) and (s._sourceGUID == UnitGUID("player")) and (JSHB.db.profile.crowdcontrol.shackleundead) then
					refreshCCTimer(9484, s._destGUID, 50)
				end
			elseif (s._subEvent == "SPELL_AURA_REMOVED") then
				-- Dominate Mind
				if (s._spellId == 605) and (s._sourceGUID == UnitGUID("player")) then
					stopCCTimer(605, s._destGUID)
				-- Psychic Scream
				elseif (s._spellId == 8122) and (s._sourceGUID == UnitGUID("player")) then
					stopAOETimer(8122)
				-- Shackle Undead
				elseif (s._spellId == 9484) and (s._sourceGUID == UnitGUID("player")) then
					stopCCTimer(9484, s._destGUID)
				-- Void Tendrils
				elseif (s._spellId == 114404) and (bit.band(s._sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == 1) then
					stopAOETimer(114404)
				end
			elseif (s._subEvent == "SPELL_AURA_BROKEN") then
				-- Void Tendrils
				if (s._spellId == 114404) and (bit.band(s._sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == 1) then
					stopAOETimer(114404)
				end
			end	
		end)
