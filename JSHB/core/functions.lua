...
function JSHB.CheckIfKnown(spell, item)
	if spell then
		if (tonumber(spell) and IsSpellKnown(spell) ) or -- Checking if the player knows the spell and it's a numeric (id)
			...
			return true
		else -- Checking if its a broken talent
			local nTiers = GetMaxTalentTier()
			local nRows = 3
			local activeSpec = GetActiveSpecGroup()
			local talentName
			
			if (tonumber(spell)) then
				talentName = select(1, GetSpellInfo(spell))
			end
			
			for tier = 1, nTiers do
				for column = 1, nRows do
					local talentID, name, iconTexture, selected, available = GetTalentInfo(tier, column, activeSpec);
					if (talentName == name) and (selected) then
						return true
					end
				end
			end
...
function JSHB.IsGlyphActive(glyph)
	local foundGlyph = false
	for i = 1, NUM_GLYPH_SLOTS do
		local enabled, glyphType, glyphTooltipIndex, glyphSpellID, icon = GetGlyphSocketInfo(i);
		if (enabled) and (glyph == glyphSpellID) then
			foundGlyph = true
		end
	end
	return foundGlyph
