	...
	for i=1,#JSHB.db.profile.timers[setsName].timers do
		...
		if ( (JSHB.db.profile.timers[setsName].timers[i][6] == 0) or (JSHB.db.profile.timers[setsName].timers[i][6] == GetSpecialization() ) ) and -- Talent tree check
			...
			if stationary then -- 3.7 addition for stationary timers
				local texture = JSHB.GetTimerIconTexture(parent.Timers[index].timerTable[1], parent.Timers[index].timerTable[2])
				if texture then
	...
	for key,val in pairs(JSHB.db.profile.timers) do
		...
		if JSHB.db.profile.timers[key].enabled and (#JSHB.db.profile.timers[key].timers > 0) then
			...
			if (not JSHB.db.profile.timers[key].stationary) then
				...
				JSHB.F.TimerSets[index]:SetSize(	JSHB.db.profile.timers[key].iconsize * (#JSHB.db.profile.timers[key].timers), JSHB.db.profile.timers[key].iconsize) -- temporary size - need to fix it later.
			end
			...
			JSHB.F.TimerSets[index]:SetScript("OnUpdate", 
				function(self, elapsed)
					...
					elseif JSHB.db.profile.timers[key].mountoverride and (IsMounted() or UnitHasVehicleUI("player") ) then
