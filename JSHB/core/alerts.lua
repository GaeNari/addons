...
local function repositionFrames(caller)
	...
	for i=1,JSHB.totalAlertIcons do
		if JSHB.F.Alerts[i].active then
			...
			caller.____xPos = ( (JSHB.db.profile.alerts.icons.iconsize + (JSHB.GetFrameOffset(JSHB.F.Alerts[1], "LEFT", 1) + JSHB.GetFrameOffset(JSHB.F.Alerts[1], "RIGHT", 1) + 2) ) * mod(caller.____index - 1, 5) )
			caller.____yPos = (JSHB.db.profile.alerts.icons.iconsize + (JSHB.GetFrameOffset(JSHB.F.Alerts[1], "TOP", 1) + JSHB.GetFrameOffset(JSHB.F.Alerts[1], "BOTTOM", 1) + 2) ) * floor( (caller.____index - 1) / 5)
...
function JSHB.SetupAlertsModule()
	...
	for key,val in pairs(JSHB.db.profile.alerts.alerts) do
		if JSHB.db.profile.alerts.alerts[key] and JSHB.db.profile.alerts.alerts[key].enabled then
			...
			-- OnUpdate handler		
			JSHB.F.Alerts[index]:SetScript("OnUpdate",
